...
SOLVE_MODEL_ODES(solve_model_odes_cpu) {
    ...
    for (int i = 0; i < num_cells_to_solve; i++) {
        sv_id = cells_to_solve ? cells_to_solve[i] : i;
        real *sv_ptr = sv + (sv_id * NEQ);

        for (int j = 0; j < num_steps; ++j) {
            int type = mapping[i];
            solve_model_ode_cpu(dt, sv_ptr, stim_currents[i], type);
        }
    }
}
...
void solve_model_ode_cpu(real dt, real *sv, real stim_current, int type_cell) {
    ...
    // Euler method
    sv[0] = dt * rDY[0] + rY[0];
    sv[1] = rDY[1]; // Rush-Larsen for v
    sv[2] = rDY[2]; // Rush-Larsen for w
    sv[3] = rDY[3]; // Rush-Larsen for s
}
...
void RHS_cpu(const real *sv, real *rDY_, real stim_current, real dt, int type_cell) {
    ...
    if (type_cell == 0) {        // ENDO
        ...
        tau_wplus = 280.0; tau_fi = 0.1; tau_o1 = 470.0; tau_o2 = 6.0; tau_so1 = 40.0;
        ...
        tau_wplus = 280.0; tau_fi = 0.078; tau_o1 = 410.0; tau_o2 = 7.0; tau_so1 = 91.0;
        ...
        tau_wplus = 200.0; tau_fi = 0.11; tau_o1 = 400.0; tau_o2 = 6.0; tau_so1 = 30.0181;
    ...
    if (tau_v_rl > 1e-10) {
        ...
        rDY_[1] = dt * ((1.0 - H) * (v_inf - v) / tau_vminus - H * v / tau_vplus) + sv[1];
    }
    ...
    if (tau_w_rl > 1e-10) {
        ...
        rDY_[2] = dt * ((1.0 - h_w) * (w_inf - w) / tau_wminus - h_w * w / tau_wplus) + sv[2];
    }
    ...
    if (tau_s > 1e-10) {
        ...
        rDY_[3] = dt * (((1.0 + tanh(k_s * (u - u_s))) * 0.5 - s) / tau_s) + sv[3];
    }
}
