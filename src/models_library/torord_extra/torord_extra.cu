...
__global__ void kernel_set_model_initial_conditions_endo_mid_epi(real *sv, int num_volumes, size_t pitch, bool use_adpt_dt, real min_dt,\
    ...
    if (threadID < num_volumes) {
        for (int i = 0; i < NEQ; i++) {
            if (transmurality[threadID] == 0) //ENDO
                *((real * )((char *) sv + pitch * i) + threadID) = initial_endo[i];
            else if (transmurality[threadID] == 2) //EPI
...
extern "C" SET_ODE_INITIAL_CONDITIONS_GPU(set_model_initial_conditions_gpu) {
    ...
    if (solver->ode_extra_data) {
        ...
        kernel_set_model_initial_conditions<<<GRID, BLOCK_SIZE>>>(solver->sv, num_volumes, pitch_h, use_adpt_dt, solver->min_dt, mapping);
    }
...
extern "C" SOLVE_MODEL_ODES(solve_model_odes_gpu) {
    ...
    if (ode_solver->ode_extra_data) {
        solve_endo_mid_epi_gpu<<<GRID, BLOCK_SIZE>>>(current_t, dt, sv, stims_currents_device, cells_to_solve_device, mapping,\
    ...
    // No transmurality: all cells ENDO
        ...
        solve_gpu<<<GRID, BLOCK_SIZE>>>(current_t, dt, sv, stims_currents_device, cells_to_solve_device, mapping,\
                                    num_cells_to_solve, num_steps, ode_solver->pitch, ode_solver->adaptive, ode_solver->abs_tol, ode_solver->rel_tol, ode_solver->max_dt);
    }
...
inline __device__ void solve_forward_euler_gpu_adpt(real *sv, real stim_curr, real transmurality, real final_time, int thread_id, size_t pitch, real abstol, real reltol, real min_dt, real max_dt) {
    ...
    RHS_gpu(sv_local, rDY, stim_curr, transmurality, thread_id, dt, pitch, true);
    time_new += dt;
...
inline __device__ void solve_rush_larsen_gpu_adpt(real *sv, real stim_curr, real transmurality, real final_time, int thread_id, size_t pitch, real abstol, real reltol, real min_dt, real max_dt) {
    ...
    RHS_RL_gpu(a_, b_, sv_local, rDY, stim_curr, transmurality, thread_id, dt, pitch, true);
    time_new += dt;
	...
	while(1) {
		...
		RHS_RL_gpu(a_new, b_new, sv_local, rDY, stim_curr, transmurality, thread_id, dt, pitch, true);
		time_new -= dt; // step back
...
inline __device__ void RHS_RL_gpu(real *a_, real *b_, real *sv, real *rDY_, real stim_current, real transmurality, int threadID_, real dt, size_t pitch, bool use_adpt_dt) {
    ...
    #include "torord_extra_RL.common.c"
}
