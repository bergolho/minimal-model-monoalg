...
SOLVE_MODEL_ODES(solve_model_odes_cpu) {
    ...
    for (u_int32_t i = 0; i < num_cells_to_solve; i++) {
        ...
        if(adpt) {
            // if (ode_solver->ode_extra_data) {
                // if (mapping[i] == 0)          // ENDO
                // else if (mapping[i] == 1)     // MID
                // else if (mapping[i] == 2)     // EPI
...
void solve_model_ode_cpu(real dt, real *sv, real stim_current, real transmurality) {
    ...
    real a[NEQ], b[NEQ];
    RHS_RL_cpu(a, b, sv, rDY, stim_current, dt, transmurality);
...
void solve_forward_euler_cpu_adpt(real *sv, real stim_curr, real transmurality, real final_time, int sv_id, struct ode_solver *solver) {
    ...
    RHS_cpu(sv, rDY, stim_curr, *dt, transmurality);
    *time_new += *dt;
...
void solve_rush_larsen_cpu_adpt(real *sv, real stim_curr, real transmurality, real final_time, int sv_id, struct ode_solver *solver) {
    ...
    RHS_RL_cpu(a_, b_, sv, rDY, stim_curr, *dt, transmurality);
    *time_new += *dt;
    ...
    while(1) {
        ...
        *time_new += *dt;
        RHS_RL_cpu(a_new, b_new, sv, rDY, stim_curr, *dt, transmurality);
        *time_new -= *dt; // step back
