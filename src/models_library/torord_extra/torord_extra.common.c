...
real delta_2 = (celltype == 2) ? 1.0-(0.95/(1.0+exp((v+70.0)/5.0))) : 1.0;
...
Gto = (celltype == 2 || celltype == 1) ? Gto*2.0 : Gto;
...
real bt=4.75;
real a_rel=0.5*bt;
real Jrel_inf=a_rel*(-ICaL)/(1.0+pow((jsr1point/cajsr),8.0));
if (celltype==1)
...
real btp=1.25*bt;
real a_relp=0.5*btp;
real Jrel_infp=a_relp*(-ICaL)/(1.0+pow((jsr1point/cajsr),8.0));
if (celltype==1)
